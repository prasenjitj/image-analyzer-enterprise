                ...
                try:
                    ...
                    try:
                        ...
                        def _normalize(s: str) -> str:
                            ...
                            suffixes = config.store_front_match_strip_suffixes_list if hasattr(
                                config, 'store_front_match_strip_suffixes_list') else []
...
class WorkerManager:
    ...
    def _recover_orphaned_chunks(self):
        """
        Recovery mechanism: Re-queue chunks that are PENDING or stuck in PROCESSING
        in the database but not in the Redis job queue. This handles scenarios where
        the worker service restarts and jobs are lost from the queue.
        """
        try:
